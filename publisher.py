...
def get_payload(message_type):
  if message_type == 'event':
    ...
    datafile.write("{}, {}, {}, {}\n".format(payload['timestamp'],payload['temperature'],payload['pressure'],payload['humidity']))
    datafile.flush()
    return payload
    ...
    payload = {
        "timestamp": time.time(),
        "Temp" : get_temp(),
        "DiskUsagePercent": get_disk_usage(),
        "MemoryUsagePercent": get_memory_usage(),
...
def mqtt_device_demo(args):
    ...
    while True:
        ...
        # delivery.
        payload = get_payload(args.message_type)


        payload_str = json.dumps(payload)

        print("Publishing {} {}: '{}'".format(args.message_type, datetime.datetime.now(), payload_str))
        client.publish(mqtt_topic, payload_str, qos=1)

        # Send events every second. State should not be updated as often
        if(args.message_type == 'state'):
